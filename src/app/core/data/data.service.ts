...
export class DataService {
  ...
  tasks = signal<Task[]>(this.offlineTasks);
  ...
  numberOfTasksAwaitingFeedback = computed(() => this.getNumberOfTasksWithStatus('Await feedback'));
  numberOfTasksInProgress = computed(() => this.getNumberOfTasksWithStatus('In progress'));
  numberOfTasksDone = computed(() => this.getNumberOfTasksWithStatus('Done'));
  numberOfUrgentTasks = computed(() => this.tasks().filter(task => task.priority === 'Urgent').length);

  filterTasks(inputString: string) {
    this.filteredTasks.set(this.tasks().filter(task => task.title.toLowerCase().includes(inputString) || task.description.toLowerCase().includes(inputString)));
  }

  getTasksNotDoneWithDeadlinesInTheFuture() {
    return this.tasks().filter(task => task.status !== 'Done' && new Date() < new Date(task.dueDate));
  }
  
  upcomingDeadline = computed(() => this.getTasksNotDoneWithDeadlinesInTheFuture().length > 0);

  closestDeadline = computed(() => {
    let filteredTasks = this.getTasksNotDoneWithDeadlinesInTheFuture();
    if (filteredTasks.length > 0) {
      let dueDate = filteredTasks.sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime())[0].dueDate;
